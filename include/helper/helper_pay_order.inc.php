...
if (!isset($_POST["orderId"]) || !is_numeric($_POST["orderId"]) || !isset($_POST["userId"]) || !is_numeric($_POST["userId"])) {
    logData("Pay Order", "Value is missing or does not have the correct datatype!", LOG_LVL_CRITICAL);
...
if (!isset($order)) {
    logData("Pay Order", "Order with id: " . $_POST["orderId"] . " not found!", LOG_LVL_CRITICAL);
...
if (!isset($user)) {
    logData("Pay Order", "User with id: " . $_POST["userId"] . " not found!", LOG_LVL_CRITICAL);
    exit(json_encode(array("state" => "error", "msg" => "user not found")));
}

if ($user->getId() != $order->getUserId()) {
    logData("Pay Order", "The order with id: " . $order->getId() . " does not belong to the user with id: " . $user->getId() . "!", LOG_LVL_CRITICAL);
...
try {
    ...
    logData("Pay Order", "Date could not be parsed!", LOG_LVL_CRITICAL);
    exit(json_encode(array("state" => "error", "msg" => "update cast error")));
}

if (!isset($order)) {
    logData("Pay Order", "Order with id: " . $_POST["orderId"] . " could not be paid!", LOG_LVL_CRITICAL);
