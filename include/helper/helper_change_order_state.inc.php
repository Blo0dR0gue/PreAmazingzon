...
if (!isset($_POST["orderId"]) || !is_numeric($_POST["orderId"]) || !isset($_POST["orderStateId"]) || !is_numeric($_POST["orderStateId"])) {
    ...
    logData("Change Order State", "Value is missing or does not have the correct datatype!", LOG_CRITICAL);
    die();
}
...
if (!isset($order)) {
    logData("Change Order State", "Order with id: " . $_POST["orderId"] . " not found!", LOG_CRITICAL);
...
if (!isset($orderState)) {
    logData("Change Order State", "Order State with id: " . $_POST["orderStateId"] . " not found!", LOG_CRITICAL);
...
try {
    ...
    logData("Change Order State", "Date could not be parsed!", LOG_CRITICAL);
    exit(json_encode(array("state" => "error", "msg" => "update cast error")));
}

if (!isset($order)) {
    logData("Change Order State", "Order " . $_POST["orderId"] . " could not be updated!", LOG_CRITICAL);
