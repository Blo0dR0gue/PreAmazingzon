...
class Order
{
    ...
    public static function getAmount(): int
    {
        ...
        if (!$stmt->execute()) {
            logData("Order Model", "Query execute error!", CRITICAL_LOG);
            return 0;
        }
        ...
        $res = $stmt->get_result();
        if ($res->num_rows === 0) {
            logData("Order Model", "No Items were found! Amount is 0.", NOTICE_LOG);
            return 0;
        }
        $res = $res->fetch_assoc();
        $stmt->close();
    ...
    public static function getAmountForUser(int $userId): int
    {
        ...
        $res = $stmt->get_result();
        if ($res->num_rows === 0) {
            logData("Order Model", "No Items were found for user " . $userId . "!", NOTICE_LOG);
            return 0;
        }
        $res = $res->fetch_assoc();
        $stmt->close();
    ...
    public static function getAllInRange(int $offset, int $amount): ?array
    {
        ...
        $res = $stmt->get_result();
        if ($res->num_rows === 0) {
            logData("Order Model", "No Items were found.", NOTICE_LOG);
            return null;
        }
