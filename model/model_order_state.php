...
class OrderState
{
    ...
    public static function getById(int $id): ?OrderState
    {
        ...
        if (!$stmt->execute()) {
            logData("Order State Model", "Query execute error! (get)", LOG_LVL_CRITICAL);
            return null;
        }
        ...
        $res = $stmt->get_result();
        if ($res->num_rows === 0) {
            logData("Order State Model", "No items were found for id: " . $id, LOG_LVL_NOTICE);
            return null;
        }
        $res = $res->fetch_assoc();
        $stmt->close();
    ...
    public static function getByName(string $label): ?OrderState
    {
        ...
        $res = $stmt->get_result();
        if ($res->num_rows === 0) {
            logData("Order State Model", "No items were found for label: " . $label, LOG_LVL_NOTICE);
            return null;
        }
        $res = $res->fetch_assoc();
        $stmt->close();
    ...
    public static function getAll(): array
    {
        ...
        if (!$result) {
            logData("Order State Model", "No items were found", LOG_LVL_NOTICE);
            return [];
        }
